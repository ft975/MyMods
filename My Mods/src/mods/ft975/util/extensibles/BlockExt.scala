...
import mods.ft975.util.AABBUtil.implicitAABB2Obj
import java.util
...
class BlockExt(id: Int, mat: Material) extends Block(id, mat) {
	...
	override def getIcon(side: Int, meta: Int): Icon = getBlockTexture(null, 0, 0, 0, side)
	...
	*/
	...
	final override def addBlockHitEffects(wrd: World, tgt: MovingObjectPosition, effRendr: EffectRenderer) = {
		val x = tgt.blockX
		val y = tgt.blockY
		val z = tgt.blockZ
		val side = tgt.sideHit

		if (wrd.getBlockId(x, y, z) == blockID) {
			val f: Float = 0.1F
			var d0 = x + rand.nextDouble * (getBlockBoundsMaxX - getBlockBoundsMinX - f * 2.0F) + f + getBlockBoundsMinX
			var d1 = y + rand.nextDouble * (getBlockBoundsMaxY - getBlockBoundsMinY - f * 2.0F) + f + getBlockBoundsMinY
			var d2 = z + rand.nextDouble * (getBlockBoundsMaxZ - getBlockBoundsMinZ - f * 2.0F) + f + getBlockBoundsMinZ

			side match {
				case 0 => d1 = y + getBlockBoundsMinY - f
				case 1 => d1 = y + getBlockBoundsMaxY + f
				case 2 => d2 = z + getBlockBoundsMinZ - f
				case 3 => d2 = z + getBlockBoundsMaxZ + f
				case 4 => d0 = x + getBlockBoundsMinX - f
				case 5 => d0 = x + getBlockBoundsMaxX + f
				case _ =>
			}
