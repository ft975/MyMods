...
abstract class TileExt extends TileEntity {
	...
	def x_=(v: Int) { xCoord = v }

	def y = yCoord

	def y_=(v: Int) { yCoord = v }

	def z = zCoord

	def z_=(v: Int) { zCoord = v }

	def metadata = worldObj.getBlockMetadata(x, y, z)

	def metadata_=(v: Byte) { worldObj.setBlockMetadataWithNotify(x, y, z, v, 2) }
